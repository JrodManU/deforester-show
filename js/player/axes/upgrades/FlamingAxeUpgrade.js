...
FlamingAxeUpgrade.prototype.applyUpgradeAtRuntime = function(playerAxe) {
	...
	playerAxe.fireEmitter.width = playerAxe.axeHead.width;
	playerAxe.fireEmitter.height = playerAxe.axeHead.height;
	...
	playerAxe.startFlame = function() {
		if(this.cooldownManager.cooldownFinished(upgradeContext.id)) {
			...
			this.fireEmitter.x = this.axeHead.world.x + (this.axeHead.width / 2) * this.game.playerManager.playerController.sprite.scale.x;
			this.fireEmitter.y = this.axeHead.world.y + this.axeHead.height / 2;
